...
class _ReservationDetailPageState extends State<ReservationDetailPage> {
  ...
  Future<void> _fetchFullData() async {
    try {
      final res = await _restauRepo.getRestaurantDetails(
  ...
  Future<void> _makeCall() async {
    final phone =
        _fullRestaurant?.phone ?? widget.reservation.restaurant?.phone;
  ...
  Widget build(BuildContext context) {
    ...
    final bool isCancel =
        res.status == ReservationStatus.annuleeResto ||
        res.status == ReservationStatus.annuleeClient;
          ...
          SliverAppBar(
            ...
            flexibleSpace: FlexibleSpaceBar(
              title: Column(
                ...
                children: [
                  Text(
                    restau?.restaurantName ?? "Réservation",
                  ...
                  if (restau != null)
                    Row(
                      children: [
                        ...
                        Text(
                          "${restau.averageRating} • ${restau.cuisineType}",
                          style: const TextStyle(
                  ...
                  if (restau?.latitude != null)
                    GoogleMap(
                      initialCameraPosition: CameraPosition(
                        target: LatLng(restau!.latitude!, restau.longitude!),
                        zoom: 16,
                      ),
                      markers: {
                        Marker(
                          ...
                          infoWindow: InfoWindow(
                            title: restau.restaurantName,
                            snippet: restau.addressString(),
                          ),
                        ),
                      },
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          _buildInfoTile(
                            ...
                            "Table",
                            res.table != null
                                ? "Table n°${res.table!.tableNumber}"
                                : "Assignation à l'arrivée",
                          ),
                          ...
                          ListTile(
                            ...
                            title: Text(
                              restau?.addressString() ?? "Chargement...",
                            ),
                          ...
                          if (restau?.description != null &&
                              restau!.description.isNotEmpty) ...[
                            const SizedBox(height: 24),
                            const Text(
                              "À propos",
                              style: TextStyle(
                          ...
                          if (res.specialRequest != null &&
                              res.specialRequest!.isNotEmpty) ...[
                            const SizedBox(height: 24),
                            const Text(
                              "Note spéciale pour le restaurateur",
                              style: TextStyle(
                                fontSize: 18,
                                fontWeight: FontWeight.bold,
                              ),
    ...
    final should = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: const Text(
              'Votre demande de réservation n\'a pas encore été traitée. Souhaitez-vous la retirer définitivement ?',
            ),
            actions: [
              TextButton(
  ...
  Future<void> _confirmCancel(BuildContext context) async {
    final should = await showDialog<bool>(
      ...
      builder:
          (ctx) => AlertDialog(
            ...
            content: const Text(
              'Votre réservation est validée. Souhaitez-vous informer le restaurant que vous ne viendrez pas ?',
            ),
            actions: [
              TextButton(
                onPressed: () => Navigator.pop(ctx, false),
    ...
    if (should == true) {
      try {
        await _resRepo.updateReservationStatus(
